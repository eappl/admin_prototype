	...
	<script src="lib/floating_header/jquery.list.min.js" ></script>
    ...
    function getMenu(menu_id){        
    	...
    	({
    		type: "GET",
            dataType: "json",
    		url: "?ctl=menu&ac=get.left.menu&menu_id="+menu_id,
    		success: function(msg)
    		{
    		    $("#side_accordion").html('<div class="push"></div>'); 
        		$(".push").before(msg['div']);
    		}
    	});    
    }
    ...
    function getRightHtml(url,id,type){
        ...
        if(id){
            if(type == 'li'){
                ...
                $("#"+id).attr("class","accordion-heading sdb_h_active");
            }
        }
    }
    ...
    function repwd(gid){
	   repwdGroupBox = divBox.showBox('?ctl=manager&ac=repwd&id=' + gid, {title:'修改用户', width:600, height:300});
    }    
    </script>	
</body>
</html>