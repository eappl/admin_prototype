...
class ManagerController extends AbstractController 
{
	...
	{
            //检查权限
            $PermissionCheck = $this->manager->checkMenuPermission('InsertManager');
            if($PermissionCheck['return'])	
            {
	...
	{
            //检查权限
            $PermissionCheck = $this->manager->checkMenuPermission('ModifyManager');
            if($PermissionCheck['return'])	
            {
                $id = $this->request->id;
                $Widget_Group = new Widget_Group();
	...
	{
            //检查权限
            $PermissionCheck = $this->manager->checkMenuPermission('RepwdManager');
            if($PermissionCheck['return'])	
            {            
                $id = $this->request->id;
                $Widget_Manager=new Widget_Manager();
	...
	{
            //检查权限
            $PermissionCheck = $this->manager->checkMenuPermission('RepwdUpdateManager');
            if($PermissionCheck['return'])	
            {   
		$id = trim($this->request->id);//用户ID
		$oldpasswd = trim($this->request->oldpasswd);//原密码
	...
	{
            //检查权限
            $PermissionCheck = $this->manager->checkMenuPermission('PwdresetManager');
            if($PermissionCheck['return'])	
            {   
		$id = $this->request->id;
	...
	{
            //检查权限
            $PermissionCheck = $this->manager->checkMenuPermission('PwdresetUpdateManager');
            if($PermissionCheck['return'])	
            {   
                $id = trim($this->request->id);//用户ID
                $newpasswd = trim($this->request->newpasswd);//新密码
